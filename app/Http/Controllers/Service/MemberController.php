...
class MemberController extends Controller
{
    public function register(Request $request)
    {
        ...
        if ($phone != '') {
            ...
            if ($tempPhone->code == $phone_code) {
                ...
                $member = new Member;
                $member->phone = $phone;
                $member->password = md5(intval('book') + intval($password));
                $member->save();
            ...
            $m3_email->to = $email;
            $m3_email->cc = 'chengxi19980829@gmail.com';
            $m3_email->subject = '学子书城验证';
            $m3_email->content = '请于24小时点击该链接完成验证. http://chengxi.book/service/validate_email'
                .'?member_id='.$member->id
                .'&code='.$uuid;
    ...
    public function login(Request $request)
    {
        ...
        if ($member == null) {
            ...
            if (md5(intval('book') + intval($password)) != $member->password) {
                $m3_result->status = 3;
